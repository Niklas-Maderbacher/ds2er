...
struct DBTableSingleLinkedListNode extract_documentation_py(FILE *py_file) {
  ...
  while (fgets(buffer, sizeof(buffer), py_file) != NULL) {
    ...
    if ((strncmp(pointer_buffer, "\"\"\"", 3) == 0) && cur_line_at_docs &&
      ...
      DBTableSingleLinkedListNode new_table_node;
      new_table_node.previous_node = &cur_table_node;
      cur_table_node = new_table_node;
    ...
    if (cur_line_at_docs && strncmp(pointer_buffer, "\"\"\"", 3) != 0 &&
        !cur_line_at_cols && !table_name_saved) {
      cur_table_node.table_name = StoreUntilSpace(pointer_buffer);
    ...
    // checks, if the line is in docs and actual at cols
      ...
      cur_col_node.column_name = StoreUntilSpace(pointer_buffer);
      cur_col_node.column_type = ExtractValueInBrackets(pointer_buffer);
      ...
      DBColumnSingleLinkedListNode new_col_node;
      new_col_node.previous_node = &cur_col_node;
      cur_col_node = new_col_node;
    }
    ...
    if (strncmp(pointer_buffer, "class", 5) == 0 && !cur_line_at_docs) {
    ...
    else if (strncmp(pointer_buffer, "Args:", 5) == 0 && cur_line_at_docs) {
      cur_line_before_cols = true;
    }
  }

  return cur_table_node;
}
